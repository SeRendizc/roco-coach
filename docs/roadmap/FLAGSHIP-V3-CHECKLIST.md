...
| RC | 内容 | 状态 |
|---|---|---|
| RC-401 | Effect/Trigger IR 增量迁移（按覆盖收益，不一次重写 68 个原语） | **IN_PROGRESS（第 1 条原语已落地）** | **先做尺子**：`roco/src/roco_env/coverage.py`（`python3 -m roco_env.coverage`）把 579 条战斗技能 + 245 条特性按 RC-403 四档定档，并把「解析不出来的原因」按频次排序（`reports/roco/rc401/effect-coverage.json`）。**关键口径**：覆盖率是「数据 × **已声明能力**」的属性 —— 未声明连击 473/824（57.4%），声明连击 **507/824（61.5%）**。**第一条原语 = 连击**（67 条技能，是除「特性未登记」外频次最高的）：静态「N连击」读进 `Parsed.hit_count`；**动态**（连击数+1/永久+1/翻倍/变为3连击）一律进 `unparsed` 如实登记；只有配置声明 `damage.multi_hit`（v3 声明、legacy/v2 没这块）才按 N 次结算，否则恒定 1 次 —— **legacy 8 条 golden 指纹全绿**。实测 3 连击伤害 19 → **57**（正好 3 倍），46 条静态连击技能被结算。判据：`roco/tests/test_effect_coverage.py`（5 条）+ `roco/tests/test_multi_hit.py`（7 条）；文档 `docs/roco/RC-401-EFFECT-COVERAGE.md`。**下一批按同一把尺子**：未被登记的 233 条特性、选择（20）、传动（13）、随机（11）|
