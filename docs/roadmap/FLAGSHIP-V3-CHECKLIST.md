...
| RC-105 | **BattleMode 驱动的合法行动裁剪与魔力（心）结算**（v3 纠偏插入项；人类 P0：「技能/聚能/换精灵/投降，不要道具与逃跑」「4 心、心没输 PVP」） | IN_PROGRESS | 新增候选配置 `mobile-s4-candidate-v3`（沿用 v2 的 `energy`/`turn_order` 逐字不变，新增 `mana{pool:4,faint_cost:1,loss_when_zero,surrender}` 与 `actions{allowed_kinds:[skill,charge,switch,surrender], forbidden_kinds:[item,escape], unknown_kinds_allowed:false}`）；引擎新增 `ACTION_CHARGE`/`ACTION_SURRENDER`、按配置裁剪合法行动、魔力归零立即判负。**legacy 逐位不变是硬门**（8 条 golden fingerprint + `test:env` 295 条）。证据：`EV-PVP-STANDARD-MANA`(MC-E08) / `EV-PVP-FAINT-MANA-LOSS`(MC-E09) 均为 `CROSS_SOURCE_SUPPORTED` ⇒ 仍是 **candidate，不许写成官方已确认** |
...
| RC-305 | 阵容工坊 UI 与工具合同（`query_owned_roster` / `evaluate_team` / `compare_change`…） | IN_PROGRESS | **架构裁定（人类 P0 反馈）**：产品面只有一页 = `src/client/roco.html`；六槽工作台 + 评估 + 右侧 Coach 栏以**可挂载模块** `src/client/team-workshop.js`（`mountTeamWorkshop(rootEl, opts)`）交付，由 `roco.js` 挂载；`workshop.html` 降级为开发夹具（**不另起临时页面**）。数据契约 `GET /api/roco/workshop` 必须直接消费 RC-302 七维 / RC-303 候选 / RC-304 环境先验，前端不得另写评估模板；`MetaPriorV1` 分布仍 `unknown` ⇒ **禁止胜率与伪精确强度**。验收：连续选六只、2～5 只时评估随阵容变化、满六出现完整诊断、1440/390 两档无溢出 |
| RC-306 | 3 秒 Serving 合同（300ms 初判 / 3s 解释 / 分段延迟 / 超时保短结论）**+ 只发 BattleMode 合法行动** | NOT_STARTED（依赖 RC-105） | Serving 必须按当前 `ruleset_config` 的 `actions.allowed_kinds` 发动作：标准 PVP = 技能 / 聚能 / 换精灵 / 投降（投降入次级），**不得**发 `item` / `escape`；越界即 fail closed。客户端只渲染引擎给的 `kind`，不自己编动作名 |
