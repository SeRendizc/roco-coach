...
| RC-106 | **六宠标准 PVP 能真的开一局**（v3 动态开局 + 绑定改 v3 + 服务/入口按模式选配置） | NOT_STARTED（v3 纠偏插入项） | 要做：① `reset`/`validate_team`/service 入口按模式读 `team_size`（当前硬编码 3v3）；② `energy.initial` 的 UNKNOWN 处理——要么等 MC-E04 录制，要么由调用方**显式**覆盖并在载荷里带 `ruleset_overrides[]`（标 `ENGINE_HYPOTHESIS` + reason），页面如实显示「候选：初始能量沿用练习局口径（未核验）」；③ `data/roco/battle-modes.json` 的 `pvp-standard-six-pet.ruleset_binding` v2 → v3（连带重生成 game-data-pack 的 ruleset_binding 与两条写死绑定的测试）；④ 六宠队伍进战斗页（当前战斗页仍是 3v3 迁移夹具） |
