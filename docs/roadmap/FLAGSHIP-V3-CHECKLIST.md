...
| RC-306 | 3 秒 Serving 合同（300ms 初判 / 3s 解释 / 分段延迟 / 超时保短结论）**+ 只发 BattleMode 合法行动** | **DONE（契约 + 度量）/ 待接线** | `src/coach/team-serving.mjs`：`serveStages({stages,clock,budgets})` —— 初判只在**必需段全部成功且未踩过 300ms** 时给出（否则 null + 逐条 reason，不发半截）；超时后不再开始新段；开始后跑过头的段记 `late`（值真但不进完整解释/短结论）；短结论只用预算内成功的段拼；缺失恒 null、绝不补默认值。`modeActionProblems()` 是 serving 边界第二道闸（第一道在引擎 RC-105）。守卫 `tests/roco-team-serving.test.js`（11 条 / 6 条必红）。度量 `npm run roco:serving` → `reports/roco/team-serving/serving.json`：真实四段跑 20 次，初判 P95 **18.8ms**（预算 300）、完整解释 P95 **37.5ms**（预算 3000）、degraded 0/20；**负向控制用真实时钟**注入 350ms/600ms 忙等证明超时分支可达；模式声明取自真配置 v3（合法 0 问题 / 泄漏 2 问题）。文档 `docs/roco/TEAM-SERVING.md`。**仍未做**：`GET /api/roco/workshop` 还是一次性同步返回，改成「先初判、后完整解释、超时降级」需要服务端 + 页面一起改 |
| RC-106 | **六宠标准 PVP 能真的开一局**（v3 动态开局 + 绑定改 v3 + 服务/入口按模式选配置） | **DONE（引擎侧）/ 待接线** | `roco/src/roco_env/overrides.py`（新）+ `reset(..., unverified_overrides=[...])`：只在配置声明 UNKNOWN 的路径生效、**不给覆盖仍 fail closed**、不写回配置（v3 的 `energy.initial` 仍是 null）、覆盖如实进 state/serialize/两个公开视图；`team_size` 按 `battle_mode` 读（v3 ⇒ 6，legacy 仍 3 且 8 条 golden 指纹不变）；`data/roco/battle-modes.json` 的绑定 v2 → **v3**（两处写死 v2 的测试改成读登记表）；`events_text.py` 补 charge/mana_loss/surrender 句子；`service.py` 的 `/battle/new` 接受 `ruleset_config_id` 与 `unverified_overrides`（缺覆盖 ⇒ 422 `unsupported_effect`）。**实测**：`test:env` 324 → **357**、六宠真对局 seed 11/12/13/21 打到 `mana_depleted` 且重放逐位相同、派生链 `--check` 5/5、三条必红反证原文进 `reports/roco/rc106/six-pet-battle.json`（11 check 全过）。**仍未做（接线）**：`POST /battle/new` 要从登记表取 `ruleset_config_id` 并带上覆盖、队伍长度按配置；`src/server/roco-service.js` 与 `roco/src/roco_env/service.py` 各有一处写死的 3 要改。接完线页面上的「魔力 / 心」才会从「未核验」变成引擎真值 |
