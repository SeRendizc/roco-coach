...
| RC-404 | 代表性回归集（18 属性 + 角色 + 速度 + 资源 + 换入离场 + 应对 + 印记 + 天气 + 迅捷 + 传动） | **DONE（首版 9 条 / 9 维度）** | `roco/src/roco_env/regression.py`（`python3 -m roco_env.regression [--check]`）→ `reports/roco/rc404/regression-set.json`。**为什么需要**：镜像对局只能压出 14 种事件，印记/天气/应对/持续状态根本不会出现——「引擎支持这些机制」在回归集之前没有证据。**两条判据**：① 空转不算通过（`expect_kinds` 逐条核对；声明「找一条带某机制的精灵」时那条技能必须真的用出来过；自动选首发按**解析出的效果类型**找而不是 desc 里的词——按词会命中「驱散印记」这种反着的机制）；② 不可达如实登记（天气：没有任何精灵的**规范配招**带 weather 效果 ⇒ 进 `unreachable`）。**实测**：damage 30 / faint 7 / mana_loss 7 / replacement 6 / **mark_added 1** / **status_added 1 + status_tick 10** / legacy 场景也在集里； 指纹 = 终局摘要 + 事件分布，`--check` 与磁盘一致；守卫 `tests/test_regression_set.py`（8 条，3 条必红）； 文档 `docs/roco/RC-404-REGRESSION-SET.md`。**边界**：9 条还小，18 属性只覆盖一部分；引擎一改指纹就该红 |
