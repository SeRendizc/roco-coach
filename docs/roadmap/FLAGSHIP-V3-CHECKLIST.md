...
| RC-306 | 3 秒 Serving 合同（300ms 初判 / 3s 解释 / 分段延迟 / 超时保短结论）**+ 只发 BattleMode 合法行动** | NOT_STARTED（依赖 RC-106） | Serving 必须按当前 `ruleset_config` 的 `actions.allowed_kinds` 发动作：标准 PVP = 技能/聚能/换精灵/投降，**不得**发 `item`/`escape`。当前页面只在**显示层**按模式隐藏 item/escape/struggle 并记账（隐藏 4 条）；引擎侧裁剪已在 RC-105 落地，但页面还没跑 v3 ⇒ 真正的端到端裁剪要等 RC-106 让页面用六宠 v3 开局 |
