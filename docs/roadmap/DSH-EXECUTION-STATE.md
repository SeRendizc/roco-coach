...
| 任务 | 状态 | 证据 |
|---|---|---|
| **W3-01 扩到 12—20 只精灵、40—60 技能** | **部分完成** | 精灵：**12/12 接入引擎**（`roco/src/roco_env/traits.py`，FULL 6 / PARTIAL 2 / REFUSED 4）；microcase **21 → 30**（`tests/evals/roco/cases/microcases-v1.jsonl`）；**技能 30 个候选，未达 40—60** —— 要扩技能需扩候选配招选择池，属下一轮 |
...
**W3-01 缺的那一项要说清**：技能数只有 30（目标 40—60）。原因是候选配招是
「每只 4 技能、机制覆盖互不重复」，12 只 × 4 = 48 个位置但去重后 30 个技能。
要再扩，得允许每只给 5—6 个候选或引入第二套配招 —— 那会改变 G02 的家族定义
（家族 key 里含配招），所以要单独一轮做，不能顺手改。
