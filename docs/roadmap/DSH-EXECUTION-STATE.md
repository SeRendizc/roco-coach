...
| 项 | 值 |
|---|---|
| HEAD | `3654ff1`（`fix(companion): 情绪话不再换来战报，拒绝不再被记录下来却不当回事（P1-3）`）。（按本文件 §2.1 的口径，文档声明的 HEAD 落后当前一两个提交是正常的：写文档本身也要一次提交。）**第 45 轮更正**：此前这一行写的是 `cf05b09`，落后了 3 个提交（`ff81037`/`4712cbf`/`3654ff1`）——`verify:state-doc` 允许落后 ≤12 个提交，所以它没报错，但这行字确实是过期的，已改成当前实际值 |
