...
> 本轮（M0+M1）**没有修改任何运行代码**，`src/game/engine.js` / `src/game/content.js` / `src/game/progression.js` / `src/coach/*`
...
| 4 | `03-IMPLEMENTATION-BRIEF.md` §2 列举数据写在 `src/game/engine.js`/`src/game/content.js`/`src/game/progression.js`/`src/coach/*` | 四个文件均存在且非空，导出面与描述相符（见第 2 节） | 一致 | 无需处理 |
...
### 2.1 `src/game/engine.js`（355 行，自创规则唯一来源）
...
### 2.3 `src/game/progression.js`（43 行，成长与存档）

| 内容 | 现场事实 | 判定 | 动作 |
|---|---|---|---|
| `TRAINING = {hp:+12生命, atk:+4攻击, speed:+3速度}` | 与 `RULES.training` 一致，`tests/rules.test.js` 逐字段断言 | **RETIRE** | 这是自创养成；手游的性格/天分/血脉/等级换算**均无来源**（已登记 `unknown`） |
...
### 2.4 `src/coach/toolbox.js`（260 行，本地 Agent 工具合同）
...
### 2.5 `src/coach/runtime.js`（377 行，上下文 / 模型调用 / 事实校验）
...
### 2.6 `src/coach/scheduler.js` + `src/coach/policy.js`（主动介入）
...
### 2.7 `src/coach/strategist.js` + `src/coach/retrieval.js` + `knowledge/`（RAG）

| 内容 | 现场事实 | 判定 | 动作 |
|---|---|---|---|
| `src/coach/strategist.js` 导出面 | `strategist` / `searchKnowledge` / `applicability` / `resolveCitation` / `verifyCitations` / `buildKnowledgePacket` / `rosterAdvice` / 以及大量 `strategistSession` / `dwellSignal` / `strategistTrigger` 等介入函数 | **ADAPT** | 检索骨架与「引用回查」保留；内容与 ruleset 过滤要换 |
| `knowledge/tactics.json` | 49 条 | **RETIRE**（移入 `knowledge/legacy/`） | 与 `TACTIC_CARDS` 同源，全部自创机制 |
| `knowledge/reference.generated.json` | 44 条，由 `scripts/build-knowledge.js` 从 `src/game/engine.js` 的 `SPECIES`/`SKILLS` 生成 | **RETIRE** | 生成器读自创数据，改数据即改卡 |
...
### 2.8 `src/coach/experience.js` / `src/coach/teacher.js` / `src/coach/companion.js` / `src/coach/memory.js`

| 内容 | 现场事实 | 判定 | 动作 |
|---|---|---|---|
| `src/coach/experience.js`（442 行） | `observe` / `trackAttention` / `shouldNudge` / `attentionText` / `watchCandidate` / `taskStamp` / `taskIsCurrent` / `markdown`（转义后才渲染有限 Markdown） | **ADAPT** | 注意力与任务新鲜度机制保留；`attentionText` 引用的自创技能名要换 |
| `src/coach/memory.js`（148 行） | 显式偏好、事件、journal（≤240）、reflections（带 evidenceIds）、`adaptiveGate`、`teachingPlan`、`coachSelfAudit` | **KEEP** | 记忆架构与 ruleset 无关；`coachSelfAudit` 让教练复盘自己是否迟到/重复/出错，值得保留 |
| `src/coach/teacher.js`（190 行） | 复盘 → 一个关键决策 → 相似练习 | **ADAPT** | 教学闭环保留；题目内容依赖自创机制 |
| `src/coach/companion.js`（2252 行，最大模块） | `companionState` 派生 momentum/lossStreak，档位 R0–R3 在三处生效（模板、`replyConstraints`、生成后克制扫描）；`PREACH` 黑名单拦截说教 | **KEEP** | 与 ruleset 无关；陪练不应因换题材而重写 |
| —— | 陪练里写死的句子（如「我们已经练过速度判断了」） | **ADAPT** | 需改成不绑定具体自创机制的表述 |

### 2.9 `src/server/index.js`（19.9k）与前端

| 内容 | 现场事实 | 判定 | 动作 |
|---|---|---|---|
| `src/server/index.js` | 静态服务 + `/api/*`；`publicAssets` 白名单（`tests/rules.test.js` 有测试断言「每个浏览器 import 的模块都在白名单里」） | **KEEP** | 白名单机制防 404 事故，必须保留 |
| `src/client/app.js`（122k）/ `src/client/index.html` / `src/client/style.css`（53.8k） | 完整可玩 UI；`VOICE_FEATURE=false` | **KEEP** | 本轮**明确不替换默认 UI**（M0/M1 边界） |
...
| 内容 | 现场事实 | 判定 | 动作 |
|---|---|---|---|
| 测试文件 | 19 个 unit 文件 + `tests/browser.test.js` / `tests/replace.test.js` | **KEEP** | **禁止为让新测试通过而删除旧测试** |
| 测试数量 | **375/375 通过** | **KEEP** | 这是回归网；新版必须与之并存而非替换 |
| `tests/evals/regression-set.json`（16 条） | 覆盖并列合理/低置信/证据缺失/应沉默 | **KEEP** | 自有说明已声明「只是回归网，不是质量评测集」，口径诚实，保留 |
| `tests/evals/tool-arguments.test.js` | 保留 R01「上一回合」/ R02 分支候选两个**已复现参数缺陷**作为回归 | **KEEP** | 回归样例化做得正确 |
| `tools/benchmark.js` / `scripts/eval-*.js` | 检索四臂、平衡矩阵、live eval、semantic eval | **ADAPT** | 评测跑法保留；数据换成手游 ruleset 后需重跑建立新基线 |
| `tests/evals/roco/cases/microcases-v1.jsonl` | **不存在** | **MISSING** | 下一轮核心产物 |
...
| 模块 | 决定 | 一句话理由 |
|---|---|---|
| `src/coach/runtime.js` 的**事实校验** | **KEEP** | 「模型说的数字必须能在回执里找到」是项目最有辨识度的资产 |
| `src/coach/runtime.js` 的**有界工具循环** | **KEEP** | 有界只读 ReAct + 停止/失败分支已实现并有测试 |
| `src/coach/toolbox.js` 的**合同与参数校验** | **KEEP** | 稳定标识不用下标；白名单参数；PVP 熔断在入口 |
| `src/coach/companion.js` / `src/coach/memory.js` | **KEEP** | 与 ruleset 无关，换题材不应重写 |
| `src/coach/policy.js` 的 PVP 边界 | **KEEP** | 只静音 pvp-live，理由与测试都在 |
| `src/game/engine.js` 的**时序骨架 + 状态版本 + 事件历史** | **KEEP（架构）** | 状态机/回放底座是真正可复用的部分 |
| 全部 **375 项测试** | **KEEP** | 反向保护；删除即失去回归能力 |
| `src/game/engine.js` 的 `SKILLS` / `SPECIES` / `TYPES` / 伤害公式 / 优先级数值 | **RETIRE** | 自创机制，**与手游无来源关系**；先隔离不删 |
| `src/game/content.js` 的 `STAGES` / `TACTIC_CARDS` | **RETIRE** | 玩家可见的自创关卡与自创数值战术卡 |
| `knowledge/*.json`（93 卡） | **RETIRE** | 全部 `game:'pet-coach'`、`rulesVersion:'0.6'`，内容基于自创机制 |
| `src/game/progression.js` 的养成数值 | **RETIRE** | 手游性格/天分/血脉换算无来源 |
| `src/coach/scheduler.js` 的触发特征 | **ADAPT** | 需要显式四档动作与「最佳/次佳动作差」等新特征 |
| `src/coach/strategist.js` 的检索与引用回查 | **ADAPT** | 骨架保留，内容与三重过滤重做 |
...
| `tests/evals/roco/cases/microcases-v1.jsonl` | **MISSING** | 下一轮产物 |
...
| 类别 | 实际动作 |
|---|---|
| **新增** | `data/roco/**`（来源台账、原始快照归档、normalized 数据、provenance、conflicts）、`scripts/roco/**`（安全 Lua 解析、schema、importer、hash、cross-check、支持矩阵、microcase 生成、notify、浏览器验收）、`docs/roco/**`（本文件 + 许可矩阵 + 冲突记录 + 支持矩阵 + microcase 计划 + 验收 checklist）、`docs/roadmap/DSH-EXECUTION-STATE.md`、`reports/roco/**`、`tests/evals/roco/**` |
...
| **未修改** | `src/game/engine.js`、`src/game/content.js`、`src/game/progression.js`、`src/game/rules.js`、`src/coach/*`、`knowledge/*`、`src/client/app.js`、`src/client/index.html`、`src/client/style.css`、`src/server/index.js`、**全部 375 项既有测试** |
