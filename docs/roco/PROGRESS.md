...
## planner 到底有没有用？两个口径都要看

规划器的质量有两把尺子，它们回答不同的问题，**不能互相替代**：

| 工具 | 口径 | 回答的问题 | 已知偏差 |
|---|---|---|---|
| `npm run roco:benchmark-planner` | **一步推演值**当正确答案 | 搜索有没有找到估值函数最偏好的那一手 | **偏向单步**：depth=2 允许与它不同，所以它不能用来比较搜索深度 |
| `npm run roco:benchmark-matches` | **整局胜负** | 让 planner 真的上场比赛，赢了没有 | 对手是 5 条启发式策略，不是真人；样本小 |

第一把尺子量出了候选裁剪的 bug（top1 0.27 → 0.59）。
第二把尺子的用法：**同一批 fixture 同时量 planner 与 greedy 基线**，
并报 Wilson 95% 区间 —— 不报区间的胜率表会把噪声读成「规划有用」。
换边（`--swapped`）用来排除「先手优势被误读成规划有效」：
引擎里 A 侧在镜像对局中并没有系统性优势（实测 22/18、21/19、16/24），
但两半都按 planner 视角记胜负，所以换边那部分可以直接相加。
